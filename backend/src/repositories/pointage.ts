...
export class pointageRepository implements InterfaceRepository<Pointage> {
  async findByEmployeAndDate(employeId: number, datePointage: Date): Promise<Pointage[]> {
  ...
  async findByEntrepriseAndDate(entrepriseId: number, dateDebut: Date, dateFin: Date): Promise<Pointage[]> {
    return mnprisma.pointage.findMany({
      where: {
        entrepriseId,
        datePointage: {
  ...
  async findByType(typePointage: TypePointage): Promise<Pointage[]> {
    return mnprisma.pointage.findMany({
      where: { typePointage },
      include: {
        employe: {
  ...
  async findByStatut(statut: StatutPointage): Promise<Pointage[]> {
    return mnprisma.pointage.findMany({
      where: { statut },
      include: {
        employe: {
  ...
  async findByEmployeAndPeriode(employeId: number, dateDebut: Date, dateFin: Date): Promise<Pointage[]> {
    return mnprisma.pointage.findMany({
      where: {
        employeId,
        datePointage: {
  ...
  async calculateHeuresTravaillees(employeId: number, dateDebut: Date, dateFin: Date): Promise<number> {
    ...
    return pointages.reduce((total: number, pointage: any) => {
  ...
  async findAllByUser(user: any, entrepriseId?: number): Promise<any[]> {
