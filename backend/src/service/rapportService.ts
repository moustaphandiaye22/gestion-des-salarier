...
export class RapportService {
  private rapportRepository = new rapportRepository();
  ...
  async getRapport(id: number) {
    ...
    if (!result) throw new Error(`Aucun rapport trouvé avec l'identifiant ${id}.`);
    return result;
  }
  ...
  async updateRapport(id: number, data: any) {
    const parsed = rapportValidator.partial().safeParse(data);
    if (!parsed.success) throw new Error('Validation échouée : ' + JSON.stringify(parsed.error.issues));
    const result = await this.rapportRepository.update(id, data);
    if (!result) throw new Error(`Impossible de mettre à jour le rapport avec l'identifiant ${id}.`);
    return result;
  }
