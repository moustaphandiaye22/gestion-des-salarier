...
export class FileService {
   ...
   */
  private async ensureQrCodesDirectory() {
    try {
  ...
  /**
    ...
    */
   async saveQrCodeImage(qrCodeDataURL: string, employeId: number, entrepriseId: number): Promise<string> {
     try {
   ...
   */
  async qrCodeImageExists(imagePath: string): Promise<boolean> {
    try {
   ...
   */
  async saveMultipleQrCodes(qrCodesData: { [employeId: number]: string }, entrepriseId: number): Promise<{ [employeId: number]: string }> {
    const savedPaths: { [employeId: number]: string } = {};
