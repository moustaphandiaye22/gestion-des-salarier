...
export class EmployeService {
  ...
  async createEmploye(data: any) {
    ...
    if ((data.roleUtilisateur === 'CAISSIER' || data.roleUtilisateur === 'VIGILE' as any) && data.motDePasse) {
  ...
  async updateEmploye(id: number, data: any) {
    ...
    if (data.motDePasse) {
      ...
      if (existingEmploye && existingEmploye.roleUtilisateur &&
          (existingEmploye.roleUtilisateur === 'CAISSIER' || existingEmploye.roleUtilisateur === 'VIGILE' as any)) {
      ...
      else if (data.roleUtilisateur === 'CAISSIER' || data.roleUtilisateur === 'VIGILE' as any) {
        updateData.motDePasse = await AuthUtils.hashPassword(data.motDePasse);
      }
    }
  ...
  async bulkCreateEmployes(employes: any[]) {
    const results = {
      success: [] as any[],
  ...
  async generateQrCode(employeId: number) {
  ...
  async generateQrCodeWithFileSave(employeId: number) {
    const employe = await this.employeRepository.findById(employeId);
    if (!employe) {
      throw new Error('Employé non trouvé');
    }
  ...
  async updatePresenceStats(employeId: number) {
    ...
    let totalRetards = 0;
    let totalHeures = 0;
    let dernierPointage: Date | null = null;
  ...
  async getEmployeStats(employeId: number) {
    const employe = await this.employeRepository.findById(employeId);
    if (!employe) {
      throw new Error('Employé non trouvé');
    }
  ...
  async generateAllQrCodesForEntreprise(entrepriseId: number) {
    const employes = await this.employeRepository.findAllByUser(
      { profil: 'ADMIN_ENTREPRISE', entrepriseId },
      entrepriseId
    );
