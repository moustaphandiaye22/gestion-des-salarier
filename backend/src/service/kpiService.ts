...
export class KpiService {
   ...
   */
  async calculateAllKPIs(entrepriseId: number | null, tableauDeBordId: number, isSuperAdmin: boolean = false): Promise<KPICalculationResult> {
   ...
   */
  private async calculateTauxAbsentéisme(entrepriseId: number, isSuperAdmin: boolean): Promise<number> {
   ...
   */
  private async calculateEvolutionMasseSalariale(entrepriseId: number, isSuperAdmin: boolean): Promise<number> {
    const currentMonth = new Date();
    const previousMonth = new Date(currentMonth.getFullYear(), currentMonth.getMonth() - 1, 1);

    const [currentData, previousData] = await Promise.all([
      (this.prisma as any).kpiData.findFirst({
        where: {
   ...
   */
  private async getPreviousMonthValue(entrepriseId: number, typeKpi: string): Promise<number> {
    try {
      const previousMonth = await (this.prisma as any).kpiData.findFirst({
        where: {
          entrepriseId,
   ...
   */
  private async saveKPIs(kpis: KPICalculationResult, tableauDeBordId: number, entrepriseId: number): Promise<void> {
    const kpiRecords = Object.entries(kpis).map(([key, data]) => ({
      nom: key,
      valeur: data.valeur,
      valeurPrecedente: data.valeurPrecedente,
      unite: data.unite,
      typeKpi: data.typeKpi as any,
    ...
    await (this.prisma as any).kpiData.createMany({
      data: kpiRecords,
      skipDuplicates: true
    });
  }
   ...
   */
  async getEvolutionData(entrepriseId: number, typeKpi: string, months: number = 6): Promise<Array<{ mois: string; valeur: number }>> {
    const endDate = new Date();
    const startDate = new Date(endDate.getFullYear(), endDate.getMonth() - months, 1);

    const evolutionData = await (this.prisma as any).kpiData.findMany({
      where: {
        entrepriseId,
        typeKpi: typeKpi as any,
        periode: 'MOIS',
    ...
    evolutionData.forEach((kpi: any) => {
      const monthKey = `${kpi.dateCalcul.getFullYear()}-${String(kpi.dateCalcul.getMonth() + 1).padStart(2, '0')}`;
      if (!monthlyData.has(monthKey) || kpi.dateCalcul > new Date(monthlyData.get(monthKey) as any)) {
  ...
  async comparePeriods(
    ...
    period2Start: Date,
    period2End: Date
  ): Promise<{ period1: number; period2: number; evolution: number }> {
    const [data1, data2] = await Promise.all([
      (this.prisma as any).kpiData.findMany({
        where: {
          entrepriseId,
          typeKpi: typeKpi as any,
          dateCalcul: {
    ...
    const avg1 = data1.length > 0 ? data1.reduce((sum: number, d: any) => sum + Number(d.valeur), 0) / data1.length : 0;
    const avg2 = data2.length > 0 ? data2.reduce((sum: number, d: any) => sum + Number(d.valeur), 0) / data2.length : 0;
