...
export class PointageService {
    ...
    async getAllPointages(user, entrepriseId) {
        if (user) {
    ...
    async getPointagesByEmploye(employeId) {
        return this.pointageRepository.findByEmployeAndDate(employeId, new Date());
    }
    async getPointagesByEmployeAndPeriode(employeId, dateDebut, dateFin) {
        return this.pointageRepository.findByEmployeAndPeriode(employeId, dateDebut, dateFin);
    }
    async getPointagesByEntrepriseAndDate(entrepriseId, dateDebut, dateFin) {
        return this.pointageRepository.findByEntrepriseAndDate(entrepriseId, dateDebut, dateFin);
    }
    async getPointagesByType(typePointage) {
        return this.pointageRepository.findByType(typePointage);
    }
    async getPointagesByStatut(statut) {
        return this.pointageRepository.findByStatut(statut);
    }
    ...
    async bulkCreatePointages(pointages) {
        const results = {
            success: [],
            errors: []
        };
    ...
    async pointerEntree(employeId, entrepriseId, lieu, ipAddress, localisation) {
    ...
    async filterPointages(filters) {
        ...
        if (filters.dateDebut && filters.dateFin) {
            return this.pointageRepository.findByEntrepriseAndDate(filters.entrepriseId, new Date(filters.dateDebut), new Date(filters.dateFin));
        }
        return this.pointageRepository.findAll();
    }
}
