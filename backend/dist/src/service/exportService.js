...
export class ExportService {
     ...
     */
    async exportDonneesAnalytiques(exportRecord) {
     ...
     */
    async exportRapportSalarial(exportRecord) {
        const { entrepriseId, parametres } = exportRecord;
     ...
     */
    async exportListeEmployes(exportRecord) {
     ...
     */
    async exportBulletinsPaie(exportRecord) {
        const { entrepriseId, parametres } = exportRecord;
        const { dateDebut, dateFin } = parametres;
        const bulletins = await this.prisma.bulletin.findMany({
            where: {
     ...
     */
    async exportPaiements(exportRecord) {
        const { entrepriseId, parametres } = exportRecord;
        const { dateDebut, dateFin } = parametres;
     ...
     */
    async exportKpiDashboard(exportRecord) {
        const { entrepriseId } = exportRecord;
        const kpis = await this.prisma.kpiData.findMany({
            where: {
                entrepriseId,
                dateCalcul: {
     ...
     */
    async saveDataToFile(data, exportRecord, typeName) {
     ...
     */
    async generatePDF(data, filePath, title) {
     ...
     */
    async getExportsUtilisateur(utilisateurId, entrepriseId, isSuperAdmin = false) {
     ...
     */
    async downloadExport(exportId, utilisateurId) {
