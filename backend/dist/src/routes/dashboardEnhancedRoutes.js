 ...
 */
router.get('/kpis/:entrepriseId/:tableauDeBordId', validateRouteParams, async (req, res) => {
    try {
        const { entrepriseId, tableauDeBordId } = req.params;
        const user = req.user;
        const isSuperAdmin = user?.role === 'SUPER_ADMIN';
        const kpis = await kpiService.calculateAllKPIs(isSuperAdmin ? null : parseInt(entrepriseId), parseInt(tableauDeBordId), isSuperAdmin);
        res.json({
            success: true,
            data: kpis,
 ...
 */
router.get('/evolution/:entrepriseId/:typeKpi', async (req, res) => {
    try {
        const { entrepriseId, typeKpi } = req.params;
        const { months = '6' } = req.query;
        const user = req.user;
        const isSuperAdmin = user?.role === 'SUPER_ADMIN';
        const evolutionData = await kpiService.getEvolutionData(isSuperAdmin ? 0 : parseInt(entrepriseId), // 0 pour indiquer toutes les entreprises
        typeKpi, parseInt(months));
        res.json({
            success: true,
            data: evolutionData,
            timestamp: new Date().toISOString()
...
router.post('/compare-periods', async (req, res) => {
    try {
        ...
        const user = req.user;
        const isSuperAdmin = user?.role === 'SUPER_ADMIN';
        const comparison = await kpiService.comparePeriods(isSuperAdmin ? 0 : parseInt(entrepriseId), // 0 pour indiquer toutes les entreprises
        typeKpi, new Date(period1Start), new Date(period1End), new Date(period2Start), new Date(period2End));
        res.json({
            success: true,
            data: comparison,
            timestamp: new Date().toISOString()
 ...
 */
router.get('/alerts/:entrepriseId/:tableauDeBordId', async (req, res) => {
    try {
        const { entrepriseId, tableauDeBordId } = req.params;
        const user = req.user;
        const isSuperAdmin = user?.role === 'SUPER_ADMIN';
        const alertes = await alerteService.getAlertesNonLues(parseInt(tableauDeBordId), isSuperAdmin ? null : parseInt(entrepriseId), isSuperAdmin);
        res.json({
            success: true,
            data: alertes,
            timestamp: new Date().toISOString()
 ...
 */
router.put('/alerts/:alerteId/read', async (req, res) => {
    try {
 ...
 */
router.post('/alerts/generate/:entrepriseId/:tableauDeBordId', async (req, res) => {
    try {
        const { entrepriseId, tableauDeBordId } = req.params;
        const user = req.user;
        const isSuperAdmin = user?.role === 'SUPER_ADMIN';
        const alertes = await alerteService.genererAlertesAutomatiques(isSuperAdmin ? null : parseInt(entrepriseId), parseInt(tableauDeBordId), isSuperAdmin);
        res.json({
            success: true,
            data: alertes,
 ...
 */
router.get('/alerts/statistics/:entrepriseId', async (req, res) => {
    try {
        const { entrepriseId } = req.params;
        const user = req.user;
        const isSuperAdmin = user?.role === 'SUPER_ADMIN';
        const statistiques = await alerteService.getStatistiquesAlertes(isSuperAdmin ? null : parseInt(entrepriseId), isSuperAdmin);
        res.json({
            success: true,
            data: statistiques,
            timestamp: new Date().toISOString()
...
router.get('/exports', async (req, res) => {
    try {
        const user = req.user;
        const isSuperAdmin = user?.role === 'SUPER_ADMIN';
        const exports = await exportService.getExportsUtilisateur(user.id, isSuperAdmin ? null : user.entrepriseId, isSuperAdmin);
        res.json({
            success: true,
            data: exports,
            timestamp: new Date().toISOString()
 ...
 */
router.get('/websocket/stats', requireRole(['SUPER_ADMIN', 'ADMIN_ENTREPRISE']), async (req, res) => {
    try {
 ...
 */
router.get('/summary/:entrepriseId', async (req, res) => {
    try {
        const { entrepriseId } = req.params;
...
//# sourceMappingURL=dashboardEnhancedRoutes.js.map